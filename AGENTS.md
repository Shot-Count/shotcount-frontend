...
- The `shotcount-web` worker is the single app gateway. All `/app/*` routes are owned by `shotcount-web`.
- The `DESKTOP_APP` service binding (`shotcount-web-desktop`) is only a transitional fallback for non-app routes that have not yet been migrated into `shotcount-web`.
